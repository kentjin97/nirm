...
//[[Rcpp::export]]
Rcpp::List rnirmfcpp(arma::mat data, const int nsample, const int nitem, const int ndim,
                  ...
                  const double pr_mean_w, const double prior_a, const double prior_b, const double fix_error,
  ...
  for(i = 0; i < nsample; i++)
    for(j = 0; j < ndim; j++) old_z(i,j) = R::rnorm(old_z_mean(i,j), fix_error);
  ...
  for(int iter = 1; iter <= niter; iter++){
    ...
    for(k = 0; k < nsample; k++){
      ...
      num = den = 0.0;
      for(j = 0; j < ndim; j++){
        num += R::dnorm4(new_z(k,j), new_z_mean(k,j), fix_error, 1);
        den += R::dnorm4(old_z(k,j), old_z_mean(k,j), fix_error, 1);
      }
    ...
    if(iter > nburn && iter % nthin == 0){
      ...
      for(k = 0; k < nsample; k++)
        for(j = 0; j < ndim; j++) mle_z += R::dnorm4(old_z(k,j), old_z_mean(k,j), fix_error, 1);
