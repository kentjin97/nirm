...
//[[Rcpp::export]]
Rcpp::List hrnirmfcpp(arma::cube data, const int nschool, const int nmax, const int nitem, const int ndim, arma::vec ncount,
                     ...
                     const double pr_mean_mu, const double pr_sd_mu, 
                     const double prior_a, const double prior_b, const double fix_error, 
  ...
  for(s = 0; s < nschool; s++)
    for(k = 0; k < ncount(s); k++)
      for(j = 0; j < ndim; j++) old_z(k,j,s) = old_z_mean(k,j,s) + fix_error * arma::randn();
  arma::dcube new_z = old_z;
  
  arma::dmat delta(nitem*(nitem-1)/2, nschool, fill::randu);
  delta = delta * 3.0;
  delta = delta - 1.5;
  arma::dvec sigma_d(nschool, fill::ones);
  ...
  arma::dvec avg_rand_eff(nschool, fill::zeros);
  arma::dvec var_rand_eff(nschool, fill::zeros);
  ...
  for(int iter = 1; iter <= niter; iter++){
    for(s = 0; s < nschool; s++){
      ...
      for(i = 0; i < nitem; i++){
        ...
        for(a = 0; a < nitem; a++){
          if(a != i){
            if(a > i){
              if(new_w_mat(a,b,s) > 0.00001) num += R::dnorm4(std::log(new_w_mat(a,i,s)), delta((a*(a-1)/2+i),s), std::sqrt(sigma_d(s)), 1);
              else num += R::dnorm4(std::log(0.00001), delta((a*(a-1)/2+i),s), std::sqrt(sigma_d(s)), 1);
              if(old_w_mat(a,b,s) > 0.00001) den += R::dnorm4(std::log(old_w_mat(a,i,s)), delta((a*(a-1)/2+i),s), std::sqrt(sigma_d(s)), 1);
              else den += R::dnorm4(std::log(0.00001), delta((a*(a-1)/2+i),s), std::sqrt(sigma_d(s)), 1);
            }
            else{
              if(new_w_mat(a,b,s) > 0.00001) num += R::dnorm4(std::log(new_w_mat(i,a,s)), delta((i*(i-1)/2+a),s), std::sqrt(sigma_d(s)), 1);
              else num += R::dnorm4(std::log(0.00001), delta((i*(i-1)/2+a),s), std::sqrt(sigma_d(s)), 1);
              if(old_w_mat(a,b,s) > 0.00001) den += R::dnorm4(std::log(old_w_mat(i,a,s)), delta((i*(i-1)/2+a),s), std::sqrt(sigma_d(s)), 1);
              else den += R::dnorm4(std::log(0.00001), delta((i*(i-1)/2+a),s), std::sqrt(sigma_d(s)), 1);
            }
          }
        }
      ...
      for(k = 0; k < ncount(s); k++){
        ...
        num = den = 0.0;
        for(j = 0; j < ndim; j++){
          num += R::dnorm4(new_z(k,j,s), old_z_mean(k,j,s), fix_error, 1);
          den += R::dnorm4(old_z(k,j,s), old_z_mean(k,j,s), fix_error, 1);
        }
    ...
    #pragma omp parallel for private(i, j, s, post_a, post_b) default(shared)
    for(s = 0; s < nschool; s++){
      post_a = prior_a + 0.5 * nitem * (nitem-1) / 2;
      post_b = prior_b;
      for(i = 1; i < nitem; i++){
        for(j = 0; j < i; j++){
          if(old_w_mat(i,j,s) > 0.00001) post_b += 0.5 * std::pow(std::log(old_w_mat(i,j,s)) - delta((i*(i-1)/2+j),s), 2.0);
          else post_b += 0.5 * std::pow(std::log(0.00001) - delta((i*(i-1)/2+j),s), 2.0);
          post_b += 0.5 * ((nitem * (nitem-1) / 2) / (nitem * (nitem-1) / 2 + 1)) * std::pow(delta(i*(i-1)/2+j,s), 2.0);
        }
      }
      sigma_d(s) = post_b * (1.0 / R::rchisq(post_a));
    }
    ...
    for(i = 1; i < nitem; i++){
      for(j = 0; j < i; j++){
        ...
        for(s = 0; s < nschool; s++) post_b += 0.5 * std::pow(delta((i*(i-1)/2+j),s) - mu(i*(i-1)/2+j), 2.0);
    ...
    for(i = 1; i < nitem; i++)
      for(j = 0; j < i; j++){
        #pragma omp parallel for private(s) default(shared)
        for(s = 0; s < nschool; s++){
          var_rand_eff(s) = 1.0 / (1.0 / sigma_m(i*(i-1)/2+j) + 1.0 / sigma_d(s));
          avg_rand_eff(s) = (1.0 / sigma_m(i*(i-1)/2+j)) * mu(i*(i-1)/2+j);
          if(old_w_mat(i,j,s) > 0.00001) avg_rand_eff(s) += (1.0 / sigma_d(s)) * std::log(old_w_mat(i,j,s));
          else avg_rand_eff(s) += (1.0 / sigma_d(s)) * std::log(0.00001);
          avg_rand_eff(s) *= var_rand_eff(s);
          delta(i*(i-1)/2+j,s) = R::rnorm(avg_rand_eff(s), std::sqrt(var_rand_eff(s)));
        }
        
        var_fix_eff = 1.0 / (1.0 / std::pow(pr_sd_mu, 2.0) + nschool / sigma_m(i*(i-1)/2+j));
        avg_fix_eff = (1.0 / std::pow(pr_sd_mu, 2.0)) * pr_mean_mu;
        // #pragma omp parallel for private(s) default(shared)
        for(s = 0; s < nschool; s++) avg_fix_eff += (1.0 / sigma_m(i*(i-1)/2+j)) * delta(i*(i-1)/2+j,s);
    ...
    if(iter > nburn && iter % nthin == 0){
      for(s = 0; s < nschool; s++){
        ...
        for(k = 0; k < ncount(s); k++)
          for(j = 0; j < ndim; j++) mle_z += R::dnorm4(old_z(k,j,s), old_z_mean(k,j,s), fix_error, 1);
      }
      ...
      for(s = 0; s < nschool; s++){
        ...
        for(i = 1; i < nitem; i++)
          for(j = 0; j < i; j++) mle_w += R::dnorm4(old_w_mat(i,j,s), delta(i*(i-1)/2+j, s), std::sqrt(sigma_d(s)), 1);
        for(i = 0; i < nitem * (nitem-1) / 2; i++) mle_w += R::dnorm4(delta(i,s), mu(i), std::sqrt(sigma_m(i)), 1);
      }
      ...
      for(i = 0; i < nschool; i++) samp_sigma_d(count,i) = std::sqrt(sigma_d(i));
      for(i = 0; i < nitem; i++){
        samp_gamma(count, i) = gamma(i);
        samp_sigma_g(count, i) = std::sqrt(sigma_g(i));
      }
      for(s = 0; s < nschool; s++)
        for(i = 0; i < nitem * (nitem - 1) / 2; i++) mean_delta(i,s) += delta(i,s) / ((niter-nburn)/nthin);
  ...
  output["z"] = samp_z;
  output["w"] = samp_w;
  output["delta"] = mean_delta;
  output["mu"] = samp_mu;
  output["gamma"] = samp_gamma;
  output["sigma_d"] = samp_sigma_d;
