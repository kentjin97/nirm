...
//[[Rcpp::export]]
Rcpp::List inirmfcpp(arma::mat data, const int nsample, const int nitem, const int ndim,
                     ...
                     const double pr_mean_z, const double prior_a, const double prior_b, const double fix_error,
  ...
  for(k = 0; k < nsample; k++)
    for(j = 0; j < ndim; j++)
      for(i = 0; i < nitem; i++)
        if(data(k,i) == 1.0) old_w_mean(i,j) += old_z(k,j) / (count_item(i) * 1.0);
  arma::dmat new_w_mean = old_w_mean;

  arma::dmat old_w(nitem, ndim, fill::zeros);
  for(i = 0; i < nitem; i++)
    for(j = 0; j < ndim; j++) old_w(i,j) = R::rnorm(old_w_mean(i,j), fix_error);
  ...
  for(int iter = 1; iter <= niter; iter++){
    ...
    for(i = 0; i < nitem; i++){
      ...
      num = den = 0.0;
      for(j = 0; j < ndim; j++){
        num += R::dnorm4(new_w(i,j), old_w_mean(i,j), fix_error, 1);
        den += R::dnorm4(old_w(i,j), old_w_mean(i,j), fix_error, 1);
      }
    ...
    if(iter > nburn && iter % nthin == 0){
      ...
      for(i = 0; i < nitem; i++)
        for(j = 0; j < ndim; j++) mle_w += R::dnorm4(old_w(i,j), old_w_mean(i,j), fix_error, 1);
