...
//[[Rcpp::export]]
Rcpp::List hrnirmcpp(arma::cube data, const int nschool, const int nmax, const int nitem, const int ndim, arma::vec ncount,
  ...
  for(s = 0; s < nschool; s++)
    for(k = 0; k < ncount(s); k++)
      for(j = 0; j < ndim; j++) old_z(k,j,s) = R::rnorm(old_z_mean(k,j,s), std::sqrt(sigma_z(s)));
  ...
  for(int iter = 1; iter <= niter; iter++){
    for(s = 0; s < nschool; s++){
      ...
      for(i = 0; i < nitem; i++){
        for(j = 0; j < ndim; j++){
          new_w(i,j,s) = R::rnorm(old_w(i,j,s), jump_w);
          for(k = 0; k < ncount(s); k++)
            if(data(k,i,s)==1.0){
