...
//[[Rcpp::export]]
Rcpp::List inirmvcpp(arma::mat data, const int nsample, const int nitem, const int ndim,
  ...
  double num, den, un, ratio, mle_z, mle_w, sigma_z = 1.0, sigma_w = 1.0;
  ...
  arma::dmat old_w_mean(nitem, ndim, fill::zeros);
  for(k = 0; k < nsample; k++)
    for(j = 0; j < ndim; j++)
      for(i = 0; i < nitem; i++)
        if(data(k,i) == 1) old_w_mean(i,j) += old_z(k,j) / (count_item[i] * 1.0);
  arma::dmat new_w_mean = old_w_mean;

  arma::dmat old_w(nitem, ndim, fill::zeros);
  for(i = 0; i < nitem; i++)
    for(j = 0; j < ndim; j++) old_w(i,j) = R::rnorm(old_w_mean(i,j), std::sqrt(sigma_w));
  ...
  for(int iter = 1; iter <= niter; iter++){
    for(k = 0; k < nsample; k++){
      for(j = 0; j < ndim; j++){
        ...
        for(i = 0; i < nitem; i++)
          if(data(k,i) == 1){
            new_w_mean(i,j) -= old_z(k,j) / (count_item(i) * 1.0);
            new_w_mean(i,j) += new_z(k,j) / (count_item(i) * 1.0);
          }
      }
      ...
      //if(option) printf("Sample-%.2d: Likelihood %.3f, Num %.3f, Den %.3f, Ratio: %.3f, Accept \n", k, update_like_samp, num, den, ratio);

      if(accept == 1){
        for(j = 0; j < ndim; j++){
          old_z(k,j) = new_z(k,j);
          for(i = 0; i < nitem; i++)
            if(data(k,i)==1.0) old_w_mean(i,j) = new_w_mean(i,j);
        }
        ...
        for(j = 0; j < ndim; j++){
          new_z(k,j) = old_z(k,j);
          for(i = 0; i < nitem; i++)
            if(data(k,i)==1.0) new_w_mean(i,j) = old_w_mean(i,j);
        }
      }
    }
    ...
    for(i = 0; i < nitem; i++){
      ...
      num = den = 0.0;
      for(j = 0; j < ndim; j++){
        num += R::dnorm4(new_w(i,j), old_w_mean(i,j), std::sqrt(sigma_w), 1);
        den += R::dnorm4(old_w(i,j), old_w_mean(i,j), std::sqrt(sigma_w), 1);
      }
    ...
    post_b = prior_b;
    for(i = 0; i < nitem; i++)
      for(j = 0; j < ndim; j++) post_b += 0.5 * std::pow(old_w(i,j) - old_w_mean(i,j), 2.0);
    ...
    if(iter > nburn && iter % nthin == 0){
      ...
      for(i = 0; i < nitem; i++)
        for(j = 0; j < ndim; j++) mle_w += R::dnorm4(old_w(i,j), old_w_mean(i,j), std::sqrt(sigma_w), 1);
    ...
    if(iter % nprint == 0){
      if(option){
        ...
        printf("% .3f % .3f\n", std::sqrt(sigma_z), std::sqrt(sigma_w));
      }
    }
  }
