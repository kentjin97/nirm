...
//[[Rcpp::export]]
Rcpp::List hinirmcpp(arma::cube data, const int nschool, const int nmax, const int nitem, const int ndim, arma::vec ncount,
                     ...
                     const double pr_mean_theta, const double pr_sd_theta, const double pr_mean_z,
  ...
  for(s = 0; s < nschool; s++)
    if(ncount(s) != nmax)
      for(i = ncount(s); i < nmax; i++) oldtheta(i,s) = 0.0; // Change
  arma::dmat newtheta = oldtheta;

  arma::dcube old_z(nmax, ndim, nschool, fill::randu);//
  old_z = old_z * 2.0;
  old_z = old_z - 1.0;
  arma::dcube new_z = old_z;
  arma::dvec sigma_z(nschool, fill::ones);

  arma::dcube old_w(nitem, ndim, nschool, fill::zeros);
  for(s = 0; s < nschool; s++)
    for(k = 0; k < ncount(s); k++)
      for(j = 0; j < ndim; j++)
        for(i = 0; i < nitem; i++)
          if(data(k,i,s) == 1.0) old_w(i,j,s) += old_z(k,j,s) / (count_item(i,s) * 1.0);
  ...
  arma::dmat  mean_z(ndim, nschool, fill::zeros);//
  ...
  for(int iter = 1; iter <= niter; iter++){
    ...
    for(s = 0; s < nschool; s++){
      ...
      for(k = 0; k < ncount(s); k++){
        for(j = 0; j < ndim; j++){
          new_z(k,j,s) = R::rnorm(old_z(k,j,s), jump_z);
          for(i = 0; i < nitem; i++){
            if(data(k,i,s)==1.0){
              new_w(i,j,s) -= old_z(k,j,s) / (count_item(i,s) * 1.0);
              new_w(i,j,s) += new_z(k,j,s) / (count_item(i,s) * 1.0);
            }
          }
        }
        ...
        num = den = 0.0;
        for(j = 0; j < ndim; j++){
          num += R::dnorm4(new_z(k,j,s), pr_mean_z, std::sqrt(sigma_z(s)), 1);
          den += R::dnorm4(old_z(k,j,s), pr_mean_z, std::sqrt(sigma_z(s)), 1);
        }
        ratio = update_like_samp + (num - den);
        // if(option) printf("ITEM-%.2d: Likelihood %.3f, Num %.3f, Den %.3f, Ratio: %.3f\n", i, update_like_samp, num, den, ratio);
        ...
        if(accept == 1){
          for(j = 0; j < ndim; j++){
            old_z(k,j,s) = new_z(k,j,s);
            for(i = 0; i < nitem; i++)
              if(data(k,i,s)==1.0) old_w(i,j,s) = new_w(i,j,s);
          }
          acc_z(k,s) += 1.0 / (niter * 1.0);
        }
        else{
          for(j = 0; j < ndim; j++){
            new_z(k,j,s) = old_z(k,j,s);
            for(i = 0; i < nitem; i++)
              if(data(k,i,s)==1.0) new_w(i,j,s) = old_w(i,j,s);
          }
        }
      }

      for(k = 0; k < ncount(s); k++)
        for(j = 0; j < ndim; j++) mean_z(j,s) += old_z(k,j,s) / ncount(s);
      post_a = prior_a + 0.5 * ncount(s) * ndim;
      post_b = prior_b;
      for(k = 0; k < ncount(s); k++)
        for(j = 0; j < ndim; j++) post_b += 0.5 * std::pow(old_z(k,j,s)-mean_z(j,s), 2.0);
    ...
    if(iter > nburn && iter % nthin == 0){
      for(s = 0; s < nschool; s++){
        ...
        for(k = 0; k < ncount(s); k++)
          for(j = 0; j < ndim; j++) mle_z += R::dnorm4(old_z(k,j,s), pr_mean_z, std::sqrt(sigma_z(s)), 1);
      }
