...
//[[Rcpp::export]]
Rcpp::List hrnirmecpp(arma::cube data, const int nschool, const int nmax, const int nitem, const int ndim, arma::vec ncount,
  ...
  for(s = 0; s < nschool; s++)
    for(i = 0; i < nitem; i++)
      for(j = 0; j < ndim; j++)
        for(k = 0; k < ncount(s); k++)
          if(data(k,i,s) == 1) old_z(k,j,s) += old_w(i,j,s) / (count_samp(k,s) * 1.0);
  ...
  for(int iter = 1; iter <= niter; iter++){
    for(s = 0; s < nschool; s++){
      ...
      for(i = 0; i < nitem; i++){
        for(j = 0; j < ndim; j++){
          new_w(i,j,s) = old_w(i,j,s) + jump_w * arma::randn();
          for(k = 0; k < ncount(s); k++)
            if(data(k,i,s)==1.0){
              new_z(k,j,s) -= old_w(i,j,s) / (count_samp(k,s) * 1.0);
              new_z(k,j,s) += new_w(i,j,s) / (count_samp(k,s) * 1.0);
            }
        }
        ...
        if(accept == 1){
          for(j = 0; j < ndim; j++){
            ...
            for(k = 0; k < ncount(s); k++)
              if(data(k,i,s) == 1.0) old_z(k,j,s) = new_z(k,j,s);
          } 
          ...
          for(j = 0; j < ndim; j++){
            ...
            for(k = 0; k < ncount(s); k++)
              if(data(k,i,s) == 1.0) new_z(k,j,s) = old_z(k,j,s);
          }
        }
      }
