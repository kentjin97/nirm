...
Rcpp::List rnirmvcpp(arma::mat data, const int nsample, const int nitem, const int ndim,
  ...
  arma::dmat old_z_mean(nsample, ndim, fill::zeros);
  for(i = 0; i < nitem; i++)
    for(j = 0; j < ndim; j++)
      for(k = 0; k < nsample; k++)
  ...
  for(i = 0; i < nsample; i++)
    for(j = 0; j < ndim; j++) old_z(i,j) = R::rnorm(old_z_mean(i,j), std::sqrt(sigma_z));
  ...
  for(int iter = 1; iter <= niter; iter++){
    ...
    for(i = 0; i < nitem; i++){
      for(j = 0; j < ndim; j++){
        ...
        for(k = 0; k < nsample; k++)
          if(data(k,i)==1){
            new_z_mean(k,j) -= old_w(i,j) / (count_samp(k) * 1.0);
      ...
      if(accept == 1){
        for(j = 0; j < ndim; j++){
          old_w(i,j) = new_w(i,j);
          for(k = 0; k < nsample; k++)
            if(data(k,i)==1) old_z_mean(k,j) = new_z_mean(k,j);
        } 
        ...
        for(j = 0; j < ndim; j++){
          new_w(i,j) = old_w(i,j);
          for(k = 0; k < nsample; k++)
            if(data(k,i)==1) new_z_mean(k,j) = old_z_mean(k,j);
        } 
      }
    }
    ...
    for(k = 0; k < nsample; k++){
      ...
      num = den = 0.0;
      for(j = 0; j < ndim; j++){
        num += R::dnorm4(new_z(k,j), old_z_mean(k,j), std::sqrt(sigma_z), 1);
        den += R::dnorm4(old_z(k,j), old_z_mean(k,j), std::sqrt(sigma_z), 1);
      }
    ...
    post_b = prior_b;
    for(k = 0; k < nsample; k++)
      for(j = 0; j < ndim; j++) post_b += 0.5 * std::pow(old_z(k,j) - old_z_mean(k,j), 2.0);
    ...
    if(iter > nburn && iter % nthin == 0){
      ...
      for(k = 0; k < nsample; k++)
        for(j = 0; j < ndim; j++) mle_z += R::dnorm4(old_z(k,j), old_z_mean(k,j), std::sqrt(sigma_z), 1);
