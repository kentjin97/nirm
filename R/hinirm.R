...
#' 
#' @export
hinirm <- function(dataset, nschool, nmax, nitem, ndim = 2, ncount,
                  niter = 30000, nburn = 5000, nthin = 5, nprint = 100, 
                  jump_beta = 0.1, jump_theta = 1.0, jump_z = 0.1,
                  pr_mean_theta = 0.0, pr_sd_theta = 10.0, pr_mean_z = 0.0,
  ...
  if((niter - nburn) %% nthin == 0){
    output = hinirmcpp(dataset, nschool, nmax, nitem, ndim, ncount, 
                       niter, nburn, nthin, nprint,
                       jump_beta, jump_theta, jump_z,
                       pr_mean_theta, pr_sd_theta, pr_mean_z,
                       pr_mean_gamma, pr_sd_gamma,
                       pr_mean_mu, pr_sd_mu, prior_a, prior_b,
                       option, cores)
    ...
    for(s in 1:nschool){
      z.star = matrix(output$z[max.address_z,(1:(ncount[s]*ndim)),s],ncol=ndim,byrow=TRUE)
      w.star = matrix(output$w[max.address_z,,s],ncol=ndim,byrow=TRUE)
      for(iter in 1:nmcmc){
    ...
    for(s in 1:nschool){
      for(i in 1:nitem){
        beta.est = mean(output$beta[,i,s])
        beta.std = sd(output$beta[,i,s])
      }
    }
    for(s in 1:nschool){
      for(k in 1:ncount[s]){
        theta.est = mean(output$theta[,k,s])
        theta.std = sd(output$theta[,k,s])
      }
    }
