...
#' 
#' @export
rnirmv <- function(dataset, nsample, nitem, ndim = 2, 
                   ...
                   pr_mean_w = 0.0, prior_a = 0.001, prior_b = 0.001, option = TRUE, cores = 1){
  
  if((niter - nburn) %% nthin == 0){
    output = rnirmvcpp(dataset, nsample, nitem, ndim, niter, nburn, nthin, nprint,
                     jump_beta, jump_theta, jump_z, jump_w,
                     pr_mean_beta, pr_sd_beta, pr_mean_theta, pr_sd_theta,
                     pr_mean_w, prior_a, prior_b, option, cores)
    
    nmcmc = as.integer((niter - nburn) / nthin)
    max.address_z = which.max(output$posterior_z)
    max.address_w = which.max(output$posterior_w)
    z.star = output$z[max.address_w,,]
    w.star = output$w[max.address_w,,]
    ...
    return(list(beta=output$beta, theta=output$theta, z=z.proc, w=w.proc, 
                ...
                sigma.z = output$sigma_z, sigma.w = output$sigma_w,
                z.estimate=z.est, w.estimate=w.est, 
                sigma.z.estimate=sigma.z, sigma.w.estimate = sigma.w,
