...
#' 
#' @export
hnirmf <- function(dataset, nschool, nmax, nitem, ndim = 2, ncount,
                  ...
                  prior_a = 0.001, prior_b = 0.001, fix_error = 0.1,
                  option = TRUE, cores = 1){
  
  if((niter - nburn) %% nthin == 0){
    output = hrnirmfcpp(dataset, nschool, nmax, nitem, ndim, ncount, 
                       niter, nburn, nthin, nprint,
                       jump_beta, jump_theta, jump_z, jump_w,
                       pr_mean_theta, pr_sd_theta, 
                       pr_mean_gamma, pr_sd_gamma,
                       pr_mean_mu, pr_sd_mu, prior_a, prior_b, fix_error, 
                       option, cores)
