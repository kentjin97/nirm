...
#' 
#' @export
rnirme <- function(dataset, nsample, nitem, ndim = 2, 
                   niter = 30000, nburn = 5000, nthin = 5, nprint = 100,
                   jump_beta = 0.2, jump_theta = 1.0, jump_w = 0.2,
                   pr_mean_beta = 0.0, pr_sd_beta = 10.0, pr_mean_theta = 0.0, pr_sd_theta = 10.0,
                   pr_mean_w = 0.0, prior_a = 0.001, prior_b = 0.001, 
                   option = TRUE, cores = 1){
  
  if((niter - nburn) %% nthin == 0){
    output = rnirmecpp(dataset, nsample, nitem, ndim, niter, nburn, nthin, nprint,
                     jump_beta, jump_theta, jump_w,
